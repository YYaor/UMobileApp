...
-(void)setItem:(NSArray *)items andSelectionBlock:(selectionBlock)block{
    _selBlock=[block copy];
    
    //Background Color
    self.backgroundColor=[UIColor clearColor];
    
    //Generating segments
    float buttonWith=self.frame.size.width/items.count;
    int i=0;
    for(NSDictionary *item in items){
        NSString *text=item[@"text"];
        NSString *icon=item[@"icon"];
        PPiButton *button = nil;
        
        if ([icon length]>0){
            button =[[[PPiButton alloc] initWithFrame:CGRectMake(buttonWith*i, 0, buttonWith, self.frame.size.height) text:text icon:icon textAttributes:nil andIconPosition:IconPositionLeft] autorelease];
        }else{
            button = [[[PPiButton alloc] initWithFrame:CGRectMake(buttonWith*i, 0, buttonWith, self.frame.size.height) text:text textAttributes:nil] autorelease];
        }
        ...
        //Adding separator
        if(i!=0){
            UIView *separatorView=[[[UIView alloc] initWithFrame:CGRectMake(i*buttonWith, 0, self.borderWidth, self.frame.size.height)] autorelease];
...
- (id)initWithFrame:(CGRect)frame items:(NSArray*)items iconPosition:(IconPosition)position andSelectionBlock:(selectionBlock)block{
    ...
    if (self) {
        ...
        for(NSDictionary *item in items){
            ...
            if ([icon length]>0){
                button =[[[PPiButton alloc] initWithFrame:CGRectMake(buttonWith*i, 0, buttonWith, frame.size.height) text:text icon:icon textAttributes:nil andIconPosition:position] autorelease];
            }else{
                button = [[[PPiButton alloc] initWithFrame:CGRectMake(buttonWith*i, 0, buttonWith, frame.size.height) text:text textAttributes:nil] autorelease];
            }
            [button addTarget:self action:@selector(segmentSelected:) forControlEvents:UIControlEventTouchUpInside];
            ...
            //Adding separator
            if(i!=0){
                UIView *separatorView=[[[UIView alloc] initWithFrame:CGRectMake(i*buttonWith, 0, self.borderWidth, frame.size.height)] autorelease];
                [self addSubview:separatorView];
                [self.separators addObject:separatorView];
            }
...
-(void)updateSegmentsFormat{
    ...
    for (PPiButton *segment in self.segments){
...
-(void)setSelectedColor:(UIColor *)selectedColor{
    if (_selectedColor) [_selectedColor release];
...
-(void)setTitle:(id)title forSegmentAtIndex:(NSUInteger)index{
    ...
    if(index<self.segments.count){
        PPiButton *segment=self.segments[index];
...
-(void)setTextAttributes:(NSDictionary *)textAttributes{
    if (_textAttributes) [_textAttributes release];
    _textAttributes=[textAttributes retain];
    [self updateSegmentsFormat];
}
-(void)setSelectedTextAttributes:(NSDictionary *)selectedTextAttributes{
    if (_selectedTextAttributes) [_selectedTextAttributes release];
    _selectedTextAttributes=[selectedTextAttributes retain];
    [self updateSegmentsFormat];
}
