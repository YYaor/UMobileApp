...
- (void)imageTap
{    
    UIView *coverView = [[[UIView alloc] initWithFrame:[[UIScreen mainScreen] bounds]] autorelease];
    coverView.backgroundColor = kBackViewColor;
    coverView.tag = kCoverViewTag;
    UITapGestureRecognizer *hiddenViewGecognizer = [[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(hiddenViewAnimation)] autorelease];
