...
@property (nonatomic , retain) UIBarButtonItem *previousBarButton;
...
- (void)setup
{
    ...
    self.previousBarButton = [[[UIBarButtonItem alloc] initWithTitle:@"上一项" style:UIBarButtonItemStyleBordered target:self action:@selector(previousButtonIsClicked:)] autorelease];
    self.nextBarButton = [[[UIBarButtonItem alloc] initWithTitle:@"下一项" style:UIBarButtonItemStyleBordered target:self action:@selector(nextButtonIsClicked:)] autorelease];
    
    UIBarButtonItem *flexBarButton = [[[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemFlexibleSpace target:nil action:nil] autorelease];
    
    UIBarButtonItem *doneBarButton = [[[UIBarButtonItem alloc] initWithTitle:@"完成" style:UIBarButtonItemStyleBordered target:self action:@selector(doneButtonIsClicked:)] autorelease];
    
    NSArray *barButtonItems = nil;
    if (self.showToolButton) {
        barButtonItems = @[self.previousBarButton, self.nextBarButton, flexBarButton, doneBarButton];
    }else{
        barButtonItems = @[flexBarButton,doneBarButton];
    }
...
- (void) doneButtonIsClicked:(id)sender
{
    ...
    if ([self.delegate respondsToSelector:@selector(doneButtonClick:)]) {
        [self.delegate performSelector:@selector(doneButtonClick:) withObject:self];
    }
}
...
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(keyboardWillHide:) name:UIKeyboardWillHideNotification object:nil];
    ...
    if (self.scrollView == nil) {
        if (self.textFieldDelegate) {
            self.scrollView = [self.textFieldDelegate performSelector:@selector(GetScrollView)];
        }
    }
