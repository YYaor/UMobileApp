...
- (void)releaseBlocksOnMainThread
{
    ...
    if (responeBlock) {
		[blocks addObject:responeBlock];
		[responeBlock release];
		responeBlock = nil;
	}
...
- (NSString *)responseString
{
    ...
    if (retStr == nil){
        ...
        retStr = [[[NSString alloc] initWithData:data encoding:enc] autorelease];
    }
    return retStr;//[[[NSString alloc] initWithBytes:[data bytes] length:[data length] encoding:[self responseEncoding]] autorelease];
}
...
- (void)reportFinished
{
    ...
    if (responeBlock) {
        responeBlock(self.responseString);
    }
	#endif
...
-(void)setResponeBlock:(ASIResponeBlock)aResponeBlock{
    [responeBlock release];
    responeBlock = [aResponeBlock copy];
}
